<?php
  ...
  $lc = $bdd->query('SELECT ID, title, filelink from loi ORDER BY ID DESC LIMIT 0,6');
?>
<div class="popup"></div>
  ...
  <?php $pg=$bdd->query('SELECT COUNT(*) AS m FROM loi');
    $d = $pg->fetch();
    $max=(ceil($d['m']/6));
...
$('.paginator').on('click', '.prev', function(){
  if(parseFloat($('.curPage').val())>1){
    ...
    $.ajax({
      type: "POST",
      url: 'php/page.php',
      data: '&opt=loip&page='+$('.curPage').val(),
      beforeSend: function(){$('.articles').html('<img src="fonts/loader.gif" />');},
      success: function(data){$('.articles').html(data);}
    });
...
<style>
  .loiArt{border-radius: 5px; cursor: pointer; margin-bottom: 10px; /*background: rgb(20, 115, 168);*/ background: #ccc;}
  .loiArt .titlearticle{font-size: 20px; text-align: center;}
</style>
