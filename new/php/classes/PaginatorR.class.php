<?php
  class PaginatorR{
    ...
    public function __construct(){}
      ...
      {
          $re=$bdd->prepare('SELECT COUNT(*) AS m FROM rapport');
          $re->execute();
          $d = $re->fetch();
      ...
      {
          if(isset($_GET['pager']) && is_numeric($_GET['pager']))
          {
            $page_num = $_GET['pager'];
          }
          else
      ...
      { 
            ...
            if($this->NbrePages($bdd) != 1)
            {
                if($this->PageActive($bdd) > 1)
                {
                    ...
                    $pagination .= '<a href="index.php?pager='.$previous.'#articlesR"><font color="firebrick"><i class="icon icon-chevron-left icon-lg"></i></font></a>&nbsp;';
                    
                    for($i = $this->PageActive($bdd) - $nbre_articles_max_avant_et_apres; $i < $this->PageActive($bdd) ; $i++)
                    {
                        if($i > 0)
                        {
                            $pagination .= '<a href="index.php?pager='.$i.'#articlesR" class="aPage">'.$i.' </a>&nbsp;';
                        }
                    }
                }
            }
            ...
            if($this->PageActive($bdd) != $this->NbrePages($bdd))
            {
                ...
                $pagination .= '&nbsp;<a href="index.php?pager='.$next.'#articlesR"><font color="firebrick"><i class="icon icon-chevron-right icon-lg"></i></font></a>&nbsp;';
            }
